  ...
  <div class="main-container">
    <div class="main-header">
        ...
        <div class="header-menu">
            ...
            <a class="main-header-link" href="Presence.php">Presence</a>
            <a class="main-header-link" href="../StatNiveau.php">Statistique par Niveau</a>
            <a class="main-header-link is-active" href="/Gestion_des_eleves/pointage/Pointage.php">Pointage de prof</a>
        </div>
    </div>
    <div class="content-wrapper">
       <div class="container-items">  
         <h1>Pointages des Profeseur</h1>
    
            <form action="enregistrer_pointage.php" method="post">
                
                <?php
                // Inclure le fichier de connexion à la base de données
                require('../db_conn.php');

                // Requête SQL pour récupérer tous les élèves du niveau 'A1'
                $sql =  "SELECT nom FROM liste_prof ";
                $stmt = $conn->query($sql);

                // Vérifie s'il y a des données à afficher
                if ($stmt->rowCount() > 0) {
                    echo '<table border="3">';
                    echo '<tr><th>Nom de professeur</th><th>Présent</th></tr>';
                    
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        echo '<tr>';
                        echo '<td>' . $row['nom'] . '</td>';                        
                        // Sélecteur pour la présence (Oui/Non)
                        echo '<td><select name="present[' . $row['nom'] . ']">';
